...
func _start_tween() -> void:
    ...
    tween = create_tween()
    tween.tween_property(parent, property, color_1, flash_speed).set_trans(transition).set_ease(easing)
    tween.set_loops()
    tween.tween_property(parent, property, color_2, flash_speed).set_trans(transition).set_ease(easing)
